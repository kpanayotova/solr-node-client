...
function Client(options){
   ...
   this.MLT_HANDLER = 'mlt';
   this.COLLECTION_API = 'admin/collections';
}
...
/**
 * Post to JSON Search API
 *
 * TODO: correct parametrization and fold within regular post method
 *
 * @param jsonQuery
 * @param callback
 * @returns {http.ClientRequest}
 */
Client.prototype.jsonSearch = function(jsonQuery,callback){

   var json = pickJSON(this.options.bigint).stringify(jsonQuery);
   var fullPath = [this.options.path,this.options.core, this.SELECT_HANDLER + '?&wt=json']
                              .filter(function(element){
                                 return element;
                              })
                              .join('/');
...
/**
 * MoreLikeThis query using dedicated handler
 *
 * @param query
 * @param callback
 */
Client.prototype.mlt = function(query,text,callback){
   if (!text) {
      return this.get(this.MLT_HANDLER,query,callback);
   }

   var parameters = query.build();

   var fullPath = [this.options.path,this.options.core, this.MLT_HANDLER + '?' + parameters +'&wt=json']
                              .filter(function(element){
                                 return element;
                              })
                              .join('/');
   ...
   return post('text/plain',params,callback);
}
...
Client.prototype.collectionStatus = function (colname, callback) {
    var query = this.createQuery();
    query.param('action', 'clusterstatus')
         .param('collection', colname)
         .param('wt', 'json');
    return this.get(this.COLLECTION_API, query, callback);
}

Client.prototype.createCollection = function (colname, config, params, callback) {
    var query = this.createQuery();
    params = params || {};
...
function postJSON(params,callback){
    return post('application/json', params, callback);
};

function post(contentType,params,callback){
   var headers = {
